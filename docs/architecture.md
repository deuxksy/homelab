...
| 102 | moni | VM | Ubuntu 24.04 | 2 | 2G | 32G | Cockpit(9090), PatchMon(8443), Pulse(10000) — 2026-08-23 중지(disk 보존) |
| 200 | heritage | LXC | Debian 12 | 4 | 4G | 50G | Caddy(9080), Homepage(Proxmox·Caddy 위젯), Jellyfin, Transmission, Aria2, Immich(2283 loopback/2284 LAN) |

## Architecture

```
walle (Proxmox VE 9.2, Tailscale: walle.bun-bull.ts.net)
├── VM 102: moni (Ubuntu 24.04 LTS, Cockpit + PatchMon + Pulse — 2026-08-23 중지, disk 보존)
...
└── LXC 200: heritage (Debian 12, Docker + Tailscale Serve)
    ...
    ├── Homepage (dashboard — Infra 그룹: Proxmox·Caddy 위젯, aria2/immich customapi 위젯)
    ├── Transmission (torrent)
    ├── Jellyfin (streaming)
    ├── Aria2 (다운로드 매니저, port 6800)
    └── Immich v3.1.0 (사진 관리, immich×3 — :2283 loopback + LAN :2284, ML 비활성)
```
...
3. `proxmox/ansible/playbooks/cockpit.yml` — moni VM에 Cockpit, PatchMon, Pulse 배포 (moni 재배포 시)
...
| `heritage/` | Docker Compose (`compose.yml`) 및 Homepage, Transmission, Jellyfin, Aria2, Immich 설정 |
