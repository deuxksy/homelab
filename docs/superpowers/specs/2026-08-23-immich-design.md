...
Heritage LXC 200에 Immich v3 풀스택(server/ML/PostgreSQL/Valkey)을 추가하고, aria2 다운로드 이미지(`/mnt/data2/torrent/downloads/aria`)를 External Library로 열람. RAM 확보를 위해 moni VM 102를 중지한다.

## 배경 — 리소스 재배분

walle (Intel N100 4코어, 7.5GB RAM)는 현재 압박 상태(host 여유 1.7GB, swap 2.3GB 사용). moni VM은 사용자 판단으로 PatchMon/Pulse 불필요화(Docker daemon 이미 다운) → VM 전체를 중지해 RAM 4GB를 회수한다.

| 항목 | 변경 | 효과 |
| :--- | :--- | :--- |
| moni VM 102 | `started = false` (중지, disk 보존) | RAM 4096MB 회수 |
| heritage LXC 200 | cores 2→4, memory 1536→4096, swap 512→1024 | Immich 풀스택 여유 |
...
Immich는 subpath 호스팅 미지원 → Caddy(443→9080) 체인과 별도로 Tailscale Serve 포트 2283 진입점을 둔다.
...
| :--- | :--- | :--- | :--- | :--- |
| immich-server | ghcr.io/immich-app/immich-server:`<릴리스 pin>` | 127.0.0.1:2283 | 1g | web 통합, `${UPLOAD_LOCATION}:/data` + `/mnt/data2/torrent/downloads/aria:/mnt/aria:ro` |
| immich-machine-learning | ghcr.io/immich-app/immich-machine-learning:`<릴리스 pin>` | 내부 3003 | 1.5g | model-cache named volume, CPU 추론 |
...
1. **OpenTofu**: `cockpit.tf` started=false, `variables.tf` heritage 리소스 증설 → `tofu apply` (LXC memory/cores는 cgroup hot-resize, 재시작 불필요 예상)
2. **Ansible**: compose.yml/.env.sops 변경 후 `ansible-playbook playbooks/heritage.yml` — 기존 rsync+복호화+compose up 라인 재사용. Tailscale Serve 태스크에 2283 추가 (reset 기반 재구성, 443→9080 유지)
3. **수동 1회 (IaC 밖)**: Admin UI → External Libraries → 라이브러리 생성 (import path `/mnt/aria`) → 스캔 트리거

Homepage 갱신: `services.yaml`에 Immich 항목 추가 (`https://heritage.bun-bull.ts.net:2283`), `docker.yaml`에서 `moni-docker` 항목 제거 (moni 정지로 인한 데드 엔트리 방지).

## 검증 체크리스트

1. `pct config 200` — cores 4 / memory 4096 확인 + `qm list` moni stopped
2. `docker compose ps` — immich 4컨테이너 healthy
3. heritage 내부 `curl -s -o /dev/null -w "%{http_code}" http://localhost:2283` → 200
4. `tailscale serve status` — 443→9080 + 2283 둘 다 존재
5. 브라우저: `https://heritage.bun-bull.ts.net:2283` 로딩 → admin 계정 생성 → external library 스캔 → aria 이미지 표시
...
| 검증 | 결과 |
| :--- | :--- |
| 공식 compose 원본 (main 브랜치) 조회 | cache 컨테이너 Valkey 9 + digest pin 확인, DB VectorChord 이미지 확인 |
| External Library 공식 문서 | `:ro` 마운트 + Admin UI 등록 + 스캔 트리거 방식 확인 |
| subpath 리버스 프록시 | 미지원 확인 → 전용 포트 2283 필요 |
| walle/heritage/moni 실측 (2026-08-23) | host 여유 1.7GB·swap 2.3GB, heritage swap 소진, moni 실사용 579MB·Docker 다운 |
